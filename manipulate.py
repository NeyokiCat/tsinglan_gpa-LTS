...
def mancall(uuid: str, ua: str):
    ...
    if result["feedbackState"] != 0:
        print("Feedback state is not 0")
        return
    if ua != result["ua"]:
        print("User agent does not match")
        Jdict = {}
        Jdict["err"] = True
        Jdict["error_en"] = "Invalid Session Info"
        Jdict["error_zh"] = "进程错误"
    ...
    def enc(pas):
        stmp = int(time.time())
        enc = (
            md5((str(md5(pas.encode()).hexdigest()).upper() + str(stmp)).encode())
            .hexdigest()
            .upper()
        )
        return stmp, enc
    ...
    def get_grade_list_url(sub, page, sid):
        ...
        return (
            "/api/LearningTask/GetList?semesterId="
            + str(sid)
            + "&subjectId="
            + sub
            + "&typeId=null&key=&beginTime="
            + start
            + "&endTime="
            + end
            + "&pageIndex="
    ...
    def get_gpa(task_arr):
        ...
        for i in range(len_):
            task_arr[i] = (
                task_arr[i][2:4]
                + [transpose(task_arr[i][4])[0][1], transpose(task_arr[i][4])[0][0]]
                + [transpose(task_arr[i][4])[1][1], transpose(task_arr[i][4])[1][0]]
            )
    ...
    async def append_subject(session, sid):
        cname = (await get(session, "/api/MemberShip/GetCurrentStudentInfo"))["data"][
            "cName"
        ]
        ename = (await get(session, "/api/MemberShip/GetCurrentStudentInfo"))["data"][
            "eName"
        ]

        sql.upsert(
            containers[0], sql.gen_user_info(username, timeOfAccess, cname, ename)
        )

        resp = await get(
            session,
            "/api/LearningTask/GetStuSubjectListForSelect?semesterId=" + str(sid),
        )
        ...
        for i in resp["data"]:
            ...
            if len(li) == 0:
                ...
                if (
                    ("AP" in s_name)
                    or ("A Level" in s_name)
                    or ("Linear Algebra" in s_name)
                    or ("AS" in s_name)
                ):
                    weight = "5.3"
                elif (
                    ("Physical Education" in s_name)
                    or (is_contains_chinese(s_name))
                    or ("PE" in s_name)
                ):
                    weight = "0.0"
                else:
                    weight = "4.3"
                course_info = sql.gen_subject_info(
                    i["subjectCode"], i["name"], i["id"], "", weight
                )
    ...
    async def main(sid):
        ...
        containers[4].upsert_item(body=sql.gen_semester(uuid, json.dumps(aget)))
        ...
        for i in semester_index:
            ...
            semester_index_translated.append(
                [
                    i["id"],
                    start_time,
                    end_time,
        ...
        async with session.get(
            get_grade_list_url("null", 1, sid), headers=header
        ) as req:
            js = await req.json()
            ...
            for i in range(count):
                task = asyncio.create_task(
                    get(session, get_grade_list_url("null", i + 1, sid))
                )
            ...
            for i in done:
                ...
                if (
                    finish == 1
                    and ret["data"]["isInSubjectScore"]
                    and ret["data"]["score"] != None
                ):
            ...
            for i in range(number_tasks):
                if not tasks_info[i][0] in subject_ids:
                    ...
                    tasks_by_subject[subject_ids.index(tasks_info[i][0])].append(
                        tasks_info[i]
                    )
