...
AES_KEY = config.settings["enc_key"]
...
def gen_user_info(
    username: str, timeofaccess: int = 0, Cname: str = "", Ename: str = ""
):
    user = {
        "id": username,
        "username": username,
...
def gen_user_permit(
    username: str, userRole: str = "user", timeofuse: int = 0, next_renewal: int = 0
):
    user_permit = {
        "id": username,
        "username": username,
...
def gen_semester(uuid: str, semester: str):
    seme = {
        "id": uuid,
        "uuid": uuid,
        "str": semester,
...
def init_database():
    ...
    CONTAINER_ID_SEMESTER = CONTAINER_PREFIX + SUFFIX + "_semester"
    containers = []
    ...
    try:
        container = db.create_container(
            id=CONTAINER_ID_SEMESTER, partition_key=PartitionKey(path="/uuid")
        )
    except exceptions.CosmosResourceExistsError:
        container = db.get_container_client(CONTAINER_ID_SEMESTER)
...
def append_session(
    ...
    if get_user_info(containers[0], username) is None:
        record = gen_session(username, uuid, password, sid, percentage_mode, ua, 0)
    ...
    if MODE == "blacklist":
        if permit_info[1] == "admin":
            ...
            operation_state = False
            record = gen_session(username, uuid, password, sid, percentage_mode, ua, -1)
        elif permit_info[3]:
            record = gen_session(username, uuid, password, sid, percentage_mode, ua, 0)
            operation_state = True
            ...
            record = gen_session(username, uuid, password, sid, percentage_mode, ua, 0)
            operation_state = True
            new_time_of_access = permit_info[2] + (
                NORMAL_LIMIT if permit_info[1] == "user" else ONHOLD_LIMIT
            )
            new_next_renewal = int(time()) + 3600 * 24
            update_session_count(
                containers, username, new_time_of_access, "add", new_next_renewal
            )
        ...
        return (
            operation_state,
            "Success" if operation_state else "Not authorized to use",
            uuid,
        )
    elif MODE == "whitelist":
        if permit_info[1] == "admin":
            ...
            operation_state = True
            new_time_of_access = permit_info[2] + (
                NORMAL_LIMIT if permit_info[1] == "wl_user" else ONHOLD_LIMIT
            )
            new_next_renewal = int(time()) + 3600 * 24
            update_session_count(
                containers, username, new_time_of_access, "add", new_next_renewal
...
def change_user_role(containers: list, username: str, userRole: str):
    if (
        userRole != "admin"
        and userRole != "user"
        and userRole != "wl_user"
        and userRole != "spamCTRL"
        and userRole != "bl_user"
    ):
...
def feedback_session(
    ...
    user_info = gen_user_info(
        username, data.get("timeOfAccess") + 1, data.get("Cname"), data.get("Ename")
    )
    ...
    session_data = gen_session(
        username,
        uuid,
        data.get("password"),
        data.get("semesterID"),
        data.get("percentage_mode"),
        data.get("ua"),
        1 if not do_err else -1,
        feedback_content,
    )
...
def update_session_count(
    ...
    permit_data = gen_user_permit(
        username,
        data.get("userRole"),
        data.get("timeOfUse") + timeofuse if operation == "add" else timeofuse,
        next_renewal,
    )
...
def run():
    ...
    do_run, _, uuid = append_session(
        containers, "marksong", "passwordofmarksong", 0, False
    )
    sleep(10)
    if do_run:
